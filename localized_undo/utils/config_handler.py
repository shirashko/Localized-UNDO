...
def load_unlearn_configs(yaml_path, base_setup_ids):
    """
    Loads base setups and expands them into a full list of LR-sweep experiments.
    """
    ...
    for base_id in base_setup_ids:
        ...
        for lr in lr_range:
            setup_id = f"{base_id}_lr_{lr:.1e}"
            config = base_cfg.copy()

            # Update LR and paths
            config['learning_rate'] = lr
            config['min_lr'] = lr  # In your script, min_lr usually follows LR

            # Path logic using PROJECT_ROOT from paths.py
            ...
            config['output_dir'] = str(
                MODEL_DIR / f"unlearned_models/{method_dir}/gemma-2-0.3B_all_arithmetic+eng_lr_{lr:.1e}")
            config['path_local_record'] = str(
                MODEL_DIR / f"local_records/unlearned_models/{method_dir}/gemma-2-0.3B_all_arithmetic+eng_lr_{lr:.1e}.txt")
            config['wandb_run_name'] = f"lr_{lr:.1e}"
